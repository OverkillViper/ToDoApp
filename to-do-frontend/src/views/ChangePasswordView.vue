    ...
    <div class="relative flex flex-grow h-full">
        <div class="m-auto w-full md:w-auto p-10">
            <div class="flex justify-center">
                <RouterLink to="/reset-password" class="text-white font-primary text-lg uppercase px-6 py-2 border-b-4 border-accent bg-secondary2">Change Password</RouterLink>
            </div>
            <form action="" class="p-6 bg-secondary mt-4 rounded-md shadow-xl" @submit.prevent="ChangePassword">
                <div class="text-accent font-primary">Current Password</div>
                <input type="password"     v-model="oldPassword" class="bg-secondary2 outline-none p-2 font-primary text-white rounded-md mt-2 w-full md:w-96 md:min-w-full" placeholder="Enter your new password">
                <div class="text-accent font-primary mt-4">New Password</div>
...
<script setup>
    ...
    async function ChangePassword() {
        ...
        if (oldPassword.value == undefined || oldPassword.value == '' ) {
            errors.value.push('Current password is empty')
        }
        else {
            await axios
            ...
            .then(response => {
                if (response.data.status !== 'match') {
                    errors.value.push('Current password is incorrect')
                }
                else {
                    console.log('match');
                }
